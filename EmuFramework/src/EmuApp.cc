			...
			onEvent = [this](ApplicationContext ctx, ApplicationEvent appEvent)
			{
				visit(overloaded
				{
					...
					[&](Input::DeviceChangeEvent &e)
					{
						...
						{
							postMessage(2, 1, std::format("{} 有一个连接错误", e.device.name()));
						}
	...
	else
	{
			...
			!autosaveManager.saveOnlyBackupMemory && stateIsOlderThanBackupMemory())
		{
			viewController().pushAndShowModal(std::make_unique<YesNoAlertView>(attachParams(),
				"自动保存状态的时间戳早于备份内存中的内容，真的要加载它吗？即使可能会导致进度丢失",
...
void EmuApp::handleOpenFileCommand(CStringView path)
{
	...
	if(name.empty())
	{
		postErrorMessage(std::format("无法访问路径名称:\n{}", path));
		return;
	}
...
void EmuApp::onSystemCreated()
{
	...
	{
		postErrorMessage(4, "没有足够的内存来保存回溯状态");
	}
...
void EmuApp::setupStaticBackupMemoryFile(FileIO &io, std::string_view ext, size_t size, uint8_t initValue) const
{
	...
	if(!io) [[unlikely]]
		throw std::runtime_error(std::format("打开错误 {}, 请验证保存路径具有写入权限", system().contentNameExt(ext)));
}
...
bool EmuApp::saveState(CStringView path)
{
	...
	{
		postErrorMessage(4, std::format("不能保存状态:\n{}", err.what()));
		return false;
	}
}
...
bool EmuApp::loadState(CStringView path)
{
	...
	catch(std::exception &err)
	{
		if(system().hasContent() && !hasWriteAccessToDir(system().contentSaveDirectory()))
			postErrorMessage(8, "保存文件夹无法访问，请在‘选项’➔‘文件路径’➔‘保存’中设置");
		else
			postErrorMessage(4, std::format("不能加载状态:\n{}", err.what()));
		return false;
	}
}
...
{
	return std::make_unique<YesNoAlertView>(attachParams(), "确认关闭当前内容？?",
		YesNoAlertView::Delegates
		{
			.onYes = [this]
			{
...
bool EmuApp::handleAppActionKeyInput(InputAction action, const Input::Event &srcEvent)
{
	...
	switch(AppKeyCode(action.code))
	{
		...
		case saveState:
		{
			...
			{
				viewController().pushAndShowModal(std::make_unique<YesNoAlertView>(attachParams(), "确认覆盖状态？",
					YesNoAlertView::Delegates
					{
						.onYes = [this]
						{
		...
		case decStateSlot:
		{
			if(!isPushed)
				break;
			system().decStateSlot();
			postMessage(1, false, std::format("状态插槽: {}", system().stateSlotName()));
			return true;
		}
		case incStateSlot:
		{
		...
		case exitApp:
		{
			if(!isPushed)
				break;
			viewController().pushAndShowModal(std::make_unique<YesNoAlertView>(attachParams(), "确认退出？",
		...
		case rewind:
		{
			...
			if(rewindManager.maxStates)
				...
				postMessage(3, false, "请在‘选项’➔‘系统’中设置回溯状态");
			break;
		}
		case softReset:
		{
