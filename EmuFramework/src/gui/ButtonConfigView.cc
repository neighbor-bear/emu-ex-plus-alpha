...
ButtonConfigView::ButtonConfigView(ViewAttachParams attach, InputManagerView &rootIMView_, const KeyCategory &cat_, InputDeviceConfig &devConf_):
	...
	reset
	{
		...
		{
			pushAndShowModal(makeView<YesNoAlertView>("真的要解绑这个分类下的所有按键吗？",
	...
	resetDefaults
	{
		...
		[this](const Input::Event &e)
		{
			pushAndShowModal(makeView<YesNoAlertView>("真的要将这个分类下的所有按键重置为默认设置吗？",
				YesNoAlertView::Delegates
				{
					.onYes = [this]
					{
				...
				if(d != &dev)
				{
					...
					else
					{
						...
						app().postMessage(7, false,
							std::format("从设备中按下一个键:\n{}\n再从这个设备中按下一个键以打开其配置菜单",
...
{
	if(e.motionEvent())
		text.resetString(std::format("按下最多3个键，释放任意键以设置:\n{}", actionStr));
	else
		text.resetString(std::format("按下最多3个键，释放任意键以设置:\n{}\n\n解绑:\n在上一个菜单中快速连续按下两次[左]键", actionStr));
	if(e.motionEvent())
	{
		initPointerUI();
	}
